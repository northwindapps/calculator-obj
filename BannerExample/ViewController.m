...
@synthesize TextArea,deglabel,ResultTable;
...
- (void)viewDidLoad {
  ...
  self.bannerView.adUnitID = @"ca-app-pub-5284441033171047/7052365618";
  ...
  [self.bannerView loadRequest:[GADRequest request]];
    ...
    //Loading Data
    NSUserDefaults *Hdata = [NSUserDefaults standardUserDefaults];
    NSArray *Htext = [Hdata objectForKey:@"Hdata"];
    HeadArray=[NSMutableArray arrayWithArray:Htext];
    ...
    UILongPressGestureRecognizer *BPress = [[UILongPressGestureRecognizer alloc] initWithTarget:self action:@selector(BPress:)];
    
    // ボタンに設定 http://qiita.com/notice_inc/items/b4425988a690e1aa4124
    [memoryA addGestureRecognizer:BPress];
    ...
    //Loading Text
    TextArea.text= [[NSUserDefaults standardUserDefaults]
...
- (void)BPress:(UILongPressGestureRecognizer *)gestureRecognizer
{
    switch (gestureRecognizer.state) {
            
        case UIGestureRecognizerStateBegan:
            [self performSelector:@selector(Brepeat:) withObject:nil afterDelay:0.2];
            break;
        ...
        case UIGestureRecognizerStateEnded:
            [NSObject cancelPreviousPerformRequestsWithTarget:self selector:@selector(Brepeat:) object:nil];
            break;
...
- (void)longPress:(UILongPressGestureRecognizer *)gestureRecognizer
{
    switch (gestureRecognizer.state) {
        ...
        case UIGestureRecognizerStateBegan:
            [self performSelector:@selector(repeat:) withObject:nil afterDelay:0.4];
            break;
...
- (void)Th0repeat:(id)sender
{
    ...
    // 0.5秒後に再度呼出
    [self performSelector:@selector(Th0repeat:) withObject:nil afterDelay:0.4];
    
}


- (void)Brepeat:(id)sender
{
    ...
    // 0.5秒後に再度呼出
    [self performSelector:@selector(Brepeat:) withObject:nil afterDelay:0.4];
...
- (void)SWrepeat:(id)sender
{
    ...
    // 0.5秒後に再度呼出
    [self performSelector:@selector(SWrepeat:) withObject:nil afterDelay:0.4];
...
- (void)PIrepeat:(id)sender
{
    ...
    // 0.5秒後に再度呼出
    [self performSelector:@selector(PIrepeat:) withObject:nil afterDelay:0.4];
...
- (void)sberepeat:(id)sender
{
    ...
    // 0.5秒後に再度呼出
    [self performSelector:@selector(sberepeat:) withObject:nil afterDelay:0.4];
...
- (void)mbsrepeat:(id)sender
{
    ...
    // 0.5秒後に再度呼出
    [self performSelector:@selector(mbsrepeat:) withObject:nil afterDelay:0.4];
...
- (void)mberepeat:(id)sender
{
    ...
    // 0.5秒後に再度呼出
    [self performSelector:@selector(mberepeat:) withObject:nil afterDelay:0.4];
...
- (void)sbsrepeat:(id)sender
{
    ...
    // 0.5秒後に再度呼出
    [self performSelector:@selector(sbsrepeat:) withObject:nil afterDelay:0.4];
...
- (void)powrepeat:(id)sender
{
    ...
    // 0.5秒後に再度呼出
    [self performSelector:@selector(powrepeat:) withObject:nil afterDelay:0.4];
...
- (void)lnrepeat:(id)sender
{
    ...
    // 0.5秒後に再度呼出
    [self performSelector:@selector(lnrepeat:) withObject:nil afterDelay:0.4];
...
- (void)logrepeat:(id)sender
{
    ...
    // 0.5秒後に再度呼出
    [self performSelector:@selector(logrepeat:) withObject:nil afterDelay:0.4];
...
- (void)arcrepeat:(id)sender
{
    ...
    // 0.5秒後に再度呼出
    [self performSelector:@selector(arcrepeat:) withObject:nil afterDelay:0.4];
...
- (void)sqrrepeat:(id)sender
{
    ...
    // 0.5秒後に再度呼出
    [self performSelector:@selector(sqrrepeat:) withObject:nil afterDelay:0.4];
...
- (void)tanrepeat:(id)sender
{
    ...
    // 0.5秒後に再度呼出
    [self performSelector:@selector(tanrepeat:) withObject:nil afterDelay:0.4];
...
- (void)cosrepeat:(id)sender
{
    ...
    // 0.5秒後に再度呼出
    [self performSelector:@selector(cosrepeat:) withObject:nil afterDelay:0.4];
...
- (void)sinrepeat:(id)sender
{
    ...
    // 0.5秒後に再度呼出
    [self performSelector:@selector(sinrepeat:) withObject:nil afterDelay:0.4];
...
- (IBAction)ENTER:(id)sender
{
            ...
            for (int i = 0; i < [temp_ARRAY count]; i++)
            {
                ...
                else
                {
                    ...
                    if ([temp_ARRAY[i] containsString:@"A"]&&[temp_ARRAY[i] containsString:@"B"])
                    {
                        ...
                        memoryBvalue = [B intValue];
                        
                        memoryBvalue++;
                        
                        B=[NSString stringWithFormat:@"%ld",(long)memoryBvalue];
                    }
    ...
    //Saving
    [[NSUserDefaults standardUserDefaults] setObject:TextArea.text forKey:@"preferenceString"];
...
-(void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    //Saving
    [[NSUserDefaults standardUserDefaults] setObject:self.TextArea.text forKey:@"preferenceString"];
    [[NSUserDefaults standardUserDefaults] synchronize];
    [TextArea resignFirstResponder];
...
textView
{
    [self.TextArea replaceRange:TextArea.selectedTextRange withText:insertingString];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    //TableViewCellこれはクラスの名前
    TableViewCell *cell = (TableViewCell *)[self.ResultTable dequeueReusableCellWithIdentifier:CellIdentifier];
    ...
    NSLog(@"NSInteger row %ld",(long)row);
    
    
    
    
    
    cell.ALabel.text = HeadArray[row];
...
- (NSInteger)numberOfSectionsInTableView:(UITableView *)tableView {
    ...
    NSInteger theNumberofSectionis1=1;
    
    return theNumberofSectionis1;
}
